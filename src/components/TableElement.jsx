...
const TableElement = ({
  ...
  const [rows, setRows] = useState(initialContent?.rows || 3)
  const [columns, setColumns] = useState(initialContent?.columns || 4)
  const [tableData, setTableData] = useState(initialContent?.data ||
    Array(3).fill(null).map(() => Array(4).fill(''))
  )
  const [hasColumnHeader, setHasColumnHeader] = useState(initialContent?.hasColumnHeader || false)
  const [hasRowHeader, setHasRowHeader] = useState(initialContent?.hasRowHeader || false)
  ...
  return (
    <div className="editor-section-container">
      ...
      <div className={`table-element-wrapper ${isFocused ? 'element-focused' : ''} ${cellFocused ? 'cell-focused' : ''}`}>
        ...
        <div className={`table-controls ${isFocused || cellFocused ? 'visible' : ''}`}>
